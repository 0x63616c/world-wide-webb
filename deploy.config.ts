...
export default stack("control-center", {
  services: [
    ...
    service("api", {
      ...
      // Join the attachable edge net so the LAN-edge proxy reaches api:4201 (CC-q002.21).
    ...
    service("storybook", {
      ...
      port: 6006,
      // CF Access (CC-cuuw): human-only host, gate behind an email OTP login.
    ...
    //     OrbStack does not forward swarm-published ports to the LAN (CC-q002.21).
    ...
    //     :42069 (CC-q002.22). Guest secrets are only entered on the SPA after the
    //     :80->TLS redirect, so they never cross the plaintext hop.
    //   - The public hostname resolves to the Mini's LAN IP via UniFi
    //     split-horizon DNS (CC-q002.15); the public wildcard hits a dead
    ...
    service("captive-portal", {
      ...
      // LAN (proven CC-q002.21). The portal is reached via the LAN-edge plain
    ...
    service("bosun-agent", {
      ...
      secrets: fromOp("Homelab", {
        ...
        // CF Access service-token client-ids (CC-cuuw) so the agent's `bosun up`
    ...
    // Captive-portal data hygiene (CC-q002.18): a daily one-shot job that purges
  ...
  // portal is exposed via that plain proxy on this net, not publishPort (CC-q002.21).
