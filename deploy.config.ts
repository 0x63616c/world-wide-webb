  ...
  services: [
    ...
    service("api", {
      ...
      secrets: fromOp("Homelab", {
        ...
        // deliver the real values. See scripts/save-home-location.sh (www-mqp).
      ...
      env: {
        ...
        // the runtime env into logs; APP_ENV can. www-rw07.
        APP_ENV: "production",
        ...
        // www-355t.7 fixed the env KEY name but left the value as
        ...
        // what actually worked before (www-9m05).
    ...
    // stays request-only (www-7d5b.1.2 → www-xjba). Now its OWN app + image
    ...
    service("worker", {
      ...
      // 384M cap for the reconcile/ingest loops (www-ke9a).
    ...
    service("media-worker", {
      ...
      // media-worker stays at 0 until the OrbStack↔NFS share is fixed (www-6mz7:
    ...
    service("captive-portal", {
      ...
      // READ-ONLY (www-q002.14): the portal never writes the cert volume. nginx
    ...
    service("bosun-agent", {
      ...
      // 192M cap for the deploy webhook receiver + in-process scheduler (www-ke9a).
      resources: { memory: "192M" },
      route: "hooks.worldwidewebb.co",
      port: 4202,
      // APP_ENV labels bosun's structured logs as production (www-rw07).
      env: {
        APP_ENV: "production",
      },
      secrets: fromOp("Homelab", {
    ...
    // (cronJob secrets, www-q002.13) and injected as the CF_Token env acme.sh's
