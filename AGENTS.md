...
- **Conventional commits with ticket id.** Every commit subject must be `type(area/CC-xxx): desc` (e.g. `feat(weather/CC-m9k): add poller`). The commit-msg guard enforces this. See CLAUDE.md for the full commit convention.
