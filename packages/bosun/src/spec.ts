...
// --- Cloudflare Access (edge auth gate, CC-cuuw) -----------------------------
//
...
export interface ServiceSpec {
  ...
  // (CC-cuuw). The app's domain derives from `route` (single source of truth);
  ...
  // the stack level via `attachableNetworks` (CC-q002.21).
  networks?: string[];
...
export interface Spec {
  ...
  // (CC-q002.21). Services opt in via their `networks` field.
  attachableNetworks?: string[];
  // Optional wildcard default-deny "floor" Access app for the stack's zone
  // (`*.worldwidewebb.co` => Block), CC-cuuw. The floor has no origin service,
...
export function stack(
  ...
  return {
    ...
    // callers (no accessFloor) yield an identical Spec to before (CC-cuuw).
    ...(opts.accessFloor ? { accessFloor: opts.accessFloor } : {}),
    // Attachable overlay networks (CC-q002.21). Omitted when unset so existing
...
export function service(
  ...
  // rather than silently creating no app (CC-cuuw).
...
export function cronJob(
  ...
  opts: {
    ...
    // an accepted tradeoff on the single-user homelab (CC-q002.13).
    secrets?: SecretRef[];
    volumes?: string[];
    placement?: string[];
  },
...
// --- Cloudflare Access builders (CC-cuuw) ------------------------------------
//
...
export function postgres(opts: {
  ...
  return {
    ...
    // pg_isready checks the agent's own localhost and always exits 2 (CC-hya3).
