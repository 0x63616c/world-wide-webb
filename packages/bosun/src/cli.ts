...
// access sync: reconcile Cloudflare Access apps (the edge auth gate, CC-cuuw)
...
// Then (3) reconcile Cloudflare Access apps (the edge auth gate, CC-cuuw).
//
...
async function reconcileCloudflare(
  ...
  // 3. Cloudflare Access — the edge auth gate (CC-cuuw). Separate advisory step:
...
// optional stack-level `accessFloor` (CC-cuuw). Advisory-guarded like routes/DNS.
//
