...
export interface ServeOptions {
  ...
  // Trigger a deploy (`bosun up`). Injected so tests don't run a real deploy.
  // `imageOverrides` is the per-image digest map from the request body (www-czg);
...
export async function handleServeRequest(req: Request, opts: ServeOptions): Promise<Response> {
  ...
  if (req.method === "POST" && url.pathname === `/deploy/${opts.stackName}`) {
    ...
    // can pin images by digest (www-czg). A missing or non-JSON body yields
...
// www-fmws: build the `docker run` that deploys via the FRESHLY-BUILT bosun image
...
export interface WebhookDeployDeps {
  ...
  // flavour so a not-yet-warm probe doesn't look like a deploy error (www-1dx).
...
const ONE_SHOT_PASS_ENV = [
  ...
  "CF_ZONE_ID",
  "CF_TUNNEL_ID",
  // CF Access reconcile (www-cuuw): the inner `bosun up` resolves the allowed
...
// Decide how to run a webhook-triggered deploy (www-fmws). Preferred path: a
...
// www-1dx). The one-shot runs a plain `bosun up`, which fails loudly on a red
