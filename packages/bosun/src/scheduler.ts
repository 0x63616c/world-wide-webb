...
//
// CRITICAL (CC-q002.22): a NAMED volume in the deployed stack is namespaced
...
export function buildJobCommand(
  ...
  // manual run-job for a no-secret job passes none (CC-q002.13).
...
// the scheduler never imports the provider directly (CC-q002.13).
...
export function startScheduler(
  ...
  // secret-bearing job with no resolver is skipped by runDueJobs (CC-q002.13).
  resolveSecret?: SecretResolver,
): () => void {
