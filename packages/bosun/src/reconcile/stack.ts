...
// Container-memory budget for the deploy VM, in MiB (CC-ke9a). The OrbStack
...
// class (CC-nqqj). Bumped from 4GB→5GB in CC-jagy; keep this in sync with the VM.
/** @public, deploy-spec budget constant; surfaced for tests + future plan checks (CC-ke9a). */
  ...
  if (totalMiB > VM_MEMORY_BUDGET_MIB) {
    throw new Error(
      `Memory overcommit: service memory limits sum to ${totalMiB} MiB, ` +
        ...
        "or raise VM_MEMORY_BUDGET_MIB only after bumping the VM (CC-jagy).",
    );
  }
}
...
export function renderStackYml(
  ...
  // Refuse the whole stack if the summed memory limits overcommit the VM (CC-ke9a).
  ...
  for (const svc of spec.services) {
    ...
    // LAN-reachable published port for a LAN-only service (CC-q002.12/.14). Emit a
    ...
    // on the Mac/LAN (the CC-q002.14 prod failure: nothing listened on the LAN :443
    ...
    // Resource caps (CC-ke9a). Emit limits.memory ONLY (a hard cap that, under
