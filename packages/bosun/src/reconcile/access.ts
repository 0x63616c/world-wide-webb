...
/** @public, part of the CloudflareAccessClient contract (createApp/updateApp params), www-cuuw */
      ...
      if (!email) {
        throw new Error(
          `reconcileAccess: allowed-email env var '${rule.envVar}' is unset, ` +
            "it must be sourced from 1Password via the agent's fromOp (www-cuuw)",
        );
      }
