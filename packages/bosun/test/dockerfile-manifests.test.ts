...
// Guard against the recurring footgun (www-q002.2): a Dockerfile that runs an
...
describe("Dockerfile workspace-manifest completeness (www-q002.2)", () => {
    ...
    it(`${rel} COPYs every workspace package.json before its frozen install`, () => {
      ...
      expect(
        ...
        `${rel} is missing a manifest COPY for: ${missing.join(", ")}. ` +
          ...
          "inside the image without it (www-q002.2).",
      ).toEqual([]);
    });
  }
});
