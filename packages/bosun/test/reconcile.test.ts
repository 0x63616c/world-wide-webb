...
describe("reconcile/secrets, create + stale computation", () => {
  it("creates declared secrets that do not exist yet", async () => {
    ...
    // Secret name is namespaced by the stack + declared name + value hash (www-8pt).
  ...
  it("reports a stack-labelled orphan as stale but does NOT remove it (prune is deferred)", async () => {
    ...
    // in-use secret here would abort the deploy mid-rename (www-8pt).
...
describe("reconcile/routes, live Cloudflare client", () => {
  ...
  // www-5ag.6: CF ingress carries no tag field, so route ownership for safe prune
  ...
  describe("ownership by origin (www-5ag.6)", () => {
    const ownership = {
...
describe("renderStackYml", () => {
  ...
  // Hashed secret names returned from reconcileSecrets (stack-namespaced, www-8pt).
  ...
  // www-nqqj: an explicit replica count renders as deploy.replicas; omitting it
  ...
  // the media-worker RCU-stall prod outage until memory limits land (www-ke9a).
  describe("replica count (www-nqqj)", () => {
    const parked: Spec = {
      stackName: "control-center",
      services: [
        {
  ...
  it("renders a named volume stack-prefixed and declares it at top-level (data persists)", () => {
    ...
    // Mount uses the stack-prefixed name so it reuses the managed volume (www-8pt).
  ...
  // www-q002.12/.14: a LAN-only service publishes a host port reachable on the
  ...
  // LAN (www-q002.14 fix, doc-backed). publishPort is orthogonal to `route`: a
  ...
  describe("publishPort host-published ports (www-q002.12)", () => {
    const portSpec: Spec = {
      stackName: "control-center",
      services: [
        {
    ...
    it("renders a long-form ingress-mode published port (LAN-reachable on OrbStack)", () => {
      ...
      // the Mac host + LAN; mode:host stays trapped in the VM (www-q002.14).
  ...
  // www-5ag.12: web/api/postgres must carry a Docker HEALTHCHECK so swarm tracks
  ...
  describe("container healthcheck (www-5ag.12)", () => {
    const hcSpec: Spec = {
      stackName: "control-center",
      services: [
        {
