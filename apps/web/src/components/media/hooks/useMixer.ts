...
/** Edit cooldown in ms — polls within this window after a local edit are ignored (CC-tavs). */
...
export function useMixer(rooms: MixerRoom[]): MixerState {
  ...
  // CC-tavs: tracks the last time a local edit (setRoomVolume / toggleMute) was
  ...
  // Re-sync vols and mutes when the rooms array changes (CC-tavs):
  //  - Seed new rooms (vol + mute from the poll, they have no local edit yet).
  //  - Prune rooms no longer in the prop (CC-ddo9.2).
  ...
  useEffect(() => {
    ...
    setVols((prev) => {
      ...
      // stable and React skips the re-render (CC-w6ug infinite-render guard).
  ...
  const setRoomVolume = useCallback(
    (uuid: string, target: number) => {
      setVols((prev) => {
        // CC-ecc2: locks are the ONLY thing that gangs faders.
        ...
        // CC-tavs: stamp lastEditAt for every room actually changed so the
  ...
  const toggleMute = useCallback((uuid: string) => {
    // CC-tavs: stamp lastEditAt so the [rooms] reconcile doesn't overwrite
