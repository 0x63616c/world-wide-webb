...
/**
 * Full portal journey matrix (www-q002.16). Every spec here drives the app
 * THROUGH the server round-trip (sendCode → verify → checkPassword → authorize),
 * which needs the tRPC client + effect-runner wiring tracked in www-q002.19. Until
 * that lands, the App's effect runner is a stub and the flow stalls at "sending",
 * so these are tagged @needs-wiring and skipped (the suite stays GREEN, not red-by-design). When www-q002.19 merges: drop the test.skip lines, read the OTP
