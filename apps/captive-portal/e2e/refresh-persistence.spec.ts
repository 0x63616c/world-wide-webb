...
/**
 * Mid-flow refresh persistence (www-q002.16). The flow persists step+form to
 * sessionStorage keyed by MAC for the verify/password steps only, and restores
 * position on reload within the code TTL (www-q002.7).
 *
 * Reaching verify/password NATURALLY needs the tRPC effect wiring (www-q002.19),
...
test.describe("refresh persistence", () => {
  ...
  test("@needs-wiring natural mid-flow refresh restores verify after a real sendCode", async ({
    ...
    // Once www-q002.19 wires the tRPC client, drive Landing → sending → verify for
    // real, reload, and assert we land back on verify with the email intact.
    test.skip(true, "blocked on www-q002.19 (tRPC client/effect-runner wiring)");
    await page.goto(`/?id=${MAC}`);
  });
});
