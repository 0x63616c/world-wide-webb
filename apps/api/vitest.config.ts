...
export default defineConfig({
  // Authoring-surface aliases (Track C, C7). apps/api's app router is assembled
  // from the folded features via `@features/*`, and each feature's api.ts
  // reaches the trpc runtime through `@app-kit/server`; these resolvers must
  // agree with tsconfig/vite/the root vitest config (check-alias-parity.sh).
  // `@app-kit/server` precedes `@app-kit` (prefix match).
  resolve: {
    alias: {
