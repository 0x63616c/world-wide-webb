...
// desired write is the only observable side-effect — CC-unxz.1).
class InsertChain {
...
describe("getControlsState", () => {
  ...
  it("CC-91bl: keeps last-known desired brightness when all lamps are off (bar greys, keeps level)", async () => {
    mockIsConfigured.mockReturnValue(true);
  ...
  it("CC-91bl: brightness is 0 only when no lamp has any known level", async () => {
    mockIsConfigured.mockReturnValue(true);
  ...
  it("lamps never report pending even when desired has not converged (CC-uq58)", async () => {
    mockIsConfigured.mockReturnValue(true);
    ...
    // desired-authoritative and never surface a pending cue (CC-uq58).
    expect(state.lamps.on).toBe(true);
    expect(state.lamps.pending).toBe(false);
  });
  ...
  it("labels fan 'Auto' when fan_mode is auto AND the AC is running (CC-pu4m)", async () => {
    mockIsConfigured.mockReturnValue(true);
  ...
  it("labels fan 'Off' when fan_mode is auto AND the AC mode is off (CC-pu4m)", async () => {
    mockIsConfigured.mockReturnValue(true);
...
describe("toggleControl", () => {
  ...
  it("writes desired (on) + command window for every lamp, with NO HA call (CC-unxz.1)", async () => {
    mockIsConfigured.mockReturnValue(true);
    mockDbSelect.mockReturnValue(makeSelectChain([]));
...
describe("controlsRouter.list", () => {
  ...
  it("lamps stay pending:false even when desired has not converged (CC-uq58)", async () => {
    mockIsConfigured.mockReturnValue(true);
    mockGetEntities.mockResolvedValue([]);
...
describe("setLampScene", () => {
  ...
  it("writes a uniform white kelvin desired on every lamp, NO HA call (CC-unxz.1)", async () => {
    mockIsConfigured.mockReturnValue(true);
    const writes = captureDesiredWrites();
...
describe("setLampBrightness", () => {
  ...
  it("writes raw brightness (0..255) desired on every lamp, NO HA call (CC-unxz.1)", async () => {
    mockIsConfigured.mockReturnValue(true);
    const writes = captureDesiredWrites();
...
// ─── setLampMode + activeScene='party' (CC-7d5b.3.4) ──────────────────────────
