...
describe("getClimate() (desired-authoritative, reads device_state — CC-unxz.2)", () => {
  ...
  it("returns the LIVE reported ambient when a stale desired carries ambient/action (CC-dnpj prod repro)", async () => {
...
describe("setClimateTarget() (writes desired, NO HA call)", () => {
  ...
  it("never writes reported-only ambient/action into desired (CC-dnpj)", async () => {
