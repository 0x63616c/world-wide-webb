/**
 * Tests for the DB-authoritative climate enforcer (CC-unxz.2).
 *
...
describe("decideClimateEnforcement (pure)", () => {
  it("seeds desired from reported when desired is null (no push), STRIPPING reported-only fields (CC-dnpj)", () => {
  ...
  // CC-qktc: the thermostat has a physical interface (wall unit + ecobee app),
  ...
  it("adopts external mode drift outside the command window (CC-qktc — last writer wins)", () => {
    const d = decideClimateEnforcement(
      {
        id: "c",
        entityId: "climate.home",
    ...
    // Adopted desired is the COMMANDABLE slice of reported (CC-dnpj sanitize).
  ...
  it("adopting while conditioning keeps the prior desired fan_mode (never absorbs the AC-asserted blower)", () => {
    const d = decideClimateEnforcement(
      ...
      // only because the compressor is running (CC-pu4m) — that is not intent.
...
describe("runClimateEnforcerCycle", () => {
  it("seeds the row on first HA sight (insert, NO push)", async () => {
    ...
    // ambient/action (CC-dnpj: they'd freeze the panel's room temp at seed time).
    expect(seed?.desiredState).toEqual({ mode: "cool", target: 70 });
    // Seeding never actuates HA.
    expect(mockCallService).not.toHaveBeenCalled();
  });

  it("self-heals a stale desired that carries reported-only ambient/action (CC-dnpj)", async () => {
  ...
  it("ADOPTS an external setpoint change outside the window: no HA call, desired := reported (CC-qktc)", async () => {
