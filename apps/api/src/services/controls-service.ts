...
interface LampState {
   ...
   * a MOOD_PALETTE colour, CC-vhht). null when no mode is set and lamps
   * disagree, are off, or show a custom colour (CC-7d5b.3.4).
   */
 ...
 *  A MOOD_PALETTE colour maps to Mood (CC-vhht): mood writes a distinct palette
 ...
 * (they differ per lamp by design, CC-vhht). The party mode overrides this from
 * the lamp_mode row (added in CC-7d5b.3.4 via deriveActiveScene's caller).
 */
 ...
 * colour and palette membership is the signature (CC-vhht). Throws when HA is
 * unconfigured.
 */
