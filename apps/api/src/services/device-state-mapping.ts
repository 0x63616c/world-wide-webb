...
export const DeviceKind = {
  ...
  Switch: "switch",
  Climate: "climate",
  // Sonos players (CC-5mek) — owned by the sonos-volume-enforcer, never HA-mapped.
  Speaker: "speaker",
} as const;
 ...
 * live reported room temp forever (CC-dnpj — the panel froze at the seed-time
...
export function mergeDeviceState(
  ...
  if (desired != null) {
    ...
    // reported-only ambient/action can never shadow the live values (CC-dnpj).
