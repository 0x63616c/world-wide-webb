 ...
 * actuator. The AC's control policy is ADOPT (CC-qktc) — the thermostat has a
 ...
 * CC-qktc); otherwise noop. Reported-only ambient/action never count as drift
...
export function decideClimateEnforcement(
  ...
  // reported-only ambient/action would freeze them in the merge overlay (CC-dnpj).
  ...
  // absorb it as the new intent (CC-qktc). The adopted desired is the commandable
  // slice of reported; while conditioning the reported fan_mode is the AC
  // asserting its blower (CC-pu4m), not intent — keep the prior desired fan.
...
async function reconcileClimate(entity: HaEntity | undefined): Promise<void> {
  ...
  // ambient/action (CC-dnpj): persist the sanitized desired with this cycle's
...
async function applyDecision(
  ...
  mapped: MappedHaState,
  now: Date,
  // CC-dnpj self-heal: when the stored desired carried reported-only fields, the
  ...
  switch (decision.kind) {
    ...
    // reported, no HA call — an external change became the new intent (CC-qktc).
    case "adopt": {
      await db
        .update(deviceState)
        .set({
