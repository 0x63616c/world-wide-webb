/**
 * Barrel of everything the worker app (@repo/worker) needs from the api domain
 * (CC-xjba). The worker package owns the scheduling framework + job registry;
 * the actual reconcile/ingest cycles, the env, and the migrator still live here
 * and are re-exported through the `@repo/api/worker` subpath so apps/worker has a
 ...
 * packages/core extraction (shared domain) will move these out of apps/api and
