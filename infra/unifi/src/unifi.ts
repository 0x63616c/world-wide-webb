// UniFi network config as Pulumi resources (CC-j934.3), via @pulumiverse/unifi
...
export function makeProvider(creds?: {
  ...
  return new unifi.Provider(
    ...
    // especially prone to this. (CC-j934.6 hardening.)
    { version: "0.2.0" },
  );
}
...
export function adoptExisting(
  ...
  // direct controller GET vs the RECON baseline. Tracked upstream: CC-2gpa.

  // guest_access: current state (portal_enabled=true, redirect_enabled=false,
  // auth=none). Adopt the CURRENT state; the external-portal flip (CC-q002.15)
...
export function createGuestVlan(provider: unifi.Provider, args: GuestVlanArgs): GuestVlanResources {
  const network = new unifi.Network(
    "www-guest",
    {
      ...
      // (isGuest + l2Isolation) plus the isolation flag below (CC-j934.3.2).
      purpose: "corporate",
      subnet: args.subnet,
      vlanId: args.vlanId,
      dhcpEnabled: true,
      ...
      // the bridged Update call 404s ("not found"), wedging the stack (CC-j934.3.2).
      multicastDns: true,
    },
    { provider },
  );
  ...
  // provider data source so no controller-internal id is hardcoded (CC-j934.3.2).
