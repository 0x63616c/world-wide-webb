...
// (CC-j934.5). The Cluster runs on a local-path PVC on the mini's SSD (NOT NFS:
...
// CC-ke9a cap: 768M limit, 384M request, 0.5 cpu reservation (DESIGN table).
...
export interface CnpgArgs {
  ...
  // CNPG operator install manifest version (CC-j934.4 preflight pin: 1.29.1).
 ...
 * Deployments (CC-j934.6) point DATABASE host at the `control-center-rw` Service
