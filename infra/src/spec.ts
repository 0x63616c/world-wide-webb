...
/** Container resource caps (www-ke9a). */
...
export interface WorkloadSpec {
  ...
  // the ESO convention `cc-secrets-<name>` (www-j934.4); override only if a
  // workload's synced Secret is named differently.
  secretName?: string;
  // imagePullSecret names for pulling private images (GHCR). The Secret(s) are
  // dockerconfigjson, built by ESO from the GHCR token (www-j934.6).
