// The scheduled jobs for the control-center k3s stack (CC-j934.7): the Pulumi-era
...
//    TLS Certificate + its renewal window (CC-j934.5), nothing to schedule here.
//
...
// The CNPG read-write Service the jobs connect to (CC-j934.5). The api default
 ...
 * (CC-j934.17); the NAS LAN IP by default. Consumed by deployCrons + the unit
 * tests; no other internal consumer.
 */
export function cronSpecs(nasNfsServer: string): CronJobSpec[] {
  return [
    // Captive-portal data hygiene (CC-q002.18): a daily one-shot running the api
...
export interface CronsArgs {
  ...
  // pod-egress no-route limit (§5c) does not apply to PV mounts. CC-j934.17.
  nasNfsServer: string;
}
