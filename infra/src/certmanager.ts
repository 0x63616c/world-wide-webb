...
// (CC-j934.5). The captive portal is LAN-only, so HTTP-01 can't reach it from
// the ACME server; DNS-01 via Cloudflare is the path (DESIGN section 1).
//
// IMPORTANT (CC-j934.5 constraint): the DNS-01 challenge writes TXT records at
...
// (CC-j934.6) mounts the same Secret for its TLS.
...
export interface CertManagerArgs {
  ...
  acmeEmail?: string;
  // cert-manager install manifest version (CC-j934.4 preflight pin: v1.20.2).
  version: string;
}
 ...
 * Certificate. The portal Deployment (CC-j934.6) mounts PORTAL_TLS_SECRET. No
...
export function installCertManager(args: CertManagerArgs): CertManagerResources {
  ...
  // key into PORTAL_TLS_SECRET; the portal Deployment mounts it (CC-j934.6).
