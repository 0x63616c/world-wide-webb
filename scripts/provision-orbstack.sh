#!/usr/bin/env bash
#
# Provisions the OrbStack VM on homelab to the spec this stack needs (CC-jagy).
...
# per-service memory caps in deploy.config.ts (CC-ke9a), NOT VM size — this is
...
# (CC-sizh) address that more directly than under-provisioning CPU would; dropping
...
# ORDERING (CC-6mz7): OrbStack must establish its file-share of the host NFS mount
...
if ! mount | grep -q " ${NFS_MOUNT} "; then
  ...
  echo "       (restarting OrbStack without the NFS mount up re-triggers CC-6mz7)" >&2
  exit 1
fi
