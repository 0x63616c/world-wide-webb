...
#      CC-w5y, or a `refs CC-w5y` trailer), AND that id is a REAL issue.
...
claude_reminder() {
  ...
  echo "  Conventional Commit subject + a real bd id (e.g. 'refs CC-w5y') in the body are mandatory."
...
# bd ids look like CC-w5y: an uppercase prefix, a dash, then alnum chars.
...
if [ "${#REFS[@]}" -eq 0 ]; then
  echo "REJECTED: no bd ticket id referenced (e.g. add a 'refs CC-w5y' trailer)."
  fail=1
