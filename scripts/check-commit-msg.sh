...
#      - the ticket id is the final scope segment (CC-w5y); it is REQUIRED
#        and MUST live in the scope (a body-only 'refs CC-w5y' does NOT count)
...
# A bd ticket id: uppercase prefix, dash, lowercase-alnum suffix (e.g. CC-w5y),
# with an optional dotted child suffix for epic children (e.g. CC-5ag.12).
...
claude_reminder() {
  echo ""
  echo "  Claude: EVERY change needs a commit subject of the form type(area/CC-xxx): desc"
  echo "  with a REAL bd ticket id in the scope — no exceptions. Area is mandatory."
  echo "  Example: feat(weather/CC-m9k): add Open-Meteo poller"
...
if ! echo "$SUBJECT" | grep -qE "$SUBJECT_RE"; then
  ...
  echo "  expected: type(area/CC-xxx)[!]: description   (type ∈ ${TYPES})"
