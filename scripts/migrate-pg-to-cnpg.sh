#!/usr/bin/env bash
# Postgres data migration: Swarm postgres -> CNPG (CC-j934.8).
#
...
step4_cnpg_ready() {
  ...
  [[ -n "$pod" ]] || die "CNPG primary not ready; bring the cluster Running first (CC-j934.5)"
...
step8_note() {
  ...
  log "          cutover (DESIGN §7 / CC-j934.9). This script's job ends at a VERIFIED restore."
}
...
run_full() {
  ...
  log "Cutover (app repoint, Swarm teardown) is a separate, deliberate step (CC-j934.9)."
}
