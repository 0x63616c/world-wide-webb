...
// every product lane (Control Center, Captive Portal, Text Your Ex, AMP) and
// prove product isolation, a Text Your Ex-only change must not rebuild or
...
// follow `<name>:`), mirroring scripts/check-amp-ci-deploy.ts.
...
const filters = {
  ...
  textyourex: filterBlock("textyourex"),
  any_app: filterBlock("any_app"),
} as const;

// 3. The Text Your Ex filter must key on its own product tree.
assert(
  filters.textyourex.some((glob) => glob.startsWith("products/text-your-ex")),
  "textyourex filter must include products/text-your-ex/**",
);

// 4. ISOLATION: a Text Your Ex-only change must not rebuild Control Center
//    (web/api/worker/mediaworker) or AMP, and must register as a TYE change.
const tyeOnly = ["products/text-your-ex/src/index.ts"] as const;
assert(blockMatchesAny(filters.textyourex, tyeOnly), "TYE-only change must set textyourex");
for (const lane of ["web", "api", "worker", "mediaworker", "amp", "captiveportal"] as const) {
  assert(
    !blockMatchesAny(filters[lane], tyeOnly),
    `TYE-only change must NOT trigger ${lane} rebuild (product isolation)`,
  );
}

// 5. CONSERVATIVE SHARED REBUILD: a shared platform change (packages/) must
//    rebuild the products that import it (Control Center web/api + AMP).
const sharedChange = ["packages/api/src/index.ts"] as const;
for (const lane of ["web", "api", "amp"] as const) {
  assert(
