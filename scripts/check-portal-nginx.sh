...
# config (CC-q002.25). Two regressions this blocks, both of which left every
...
if [ -f "$LOCATIONS_CONF" ]; then
  ...
  if grep -qE 'proxy_pass[[:space:]]+https?://\$' "$LOCATIONS_CONF"; then
    err "$LOCATIONS_CONF: proxy_pass uses a \$variable upstream; use a literal api Service host so it can't end up unset (CC-q002.25)"
  fi
  ...
  #    gzipped captive page ("the webpage couldn't be loaded"). CC-q002.26.
  if grep -qE '^[[:space:]]*gzip[[:space:]]+on;' "$LOCATIONS_CONF"; then
    err "$LOCATIONS_CONF: 'gzip on' breaks the Apple captive sheet; keep gzip off for the portal (CC-q002.26)"
  fi
fi
