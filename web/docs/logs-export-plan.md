...
- Add to `web/package.json` dependencies via
  `bun add @capacitor/share@^8` (run inside `web`), so the
...
**Modify:**
- `web/package.json` — add `@capacitor/share@^8` dependency.
- `web/bun.lock` — regenerated by `bun add`.
- `web/src/lib/log/native.ts` — add `getUri` to the
  `LogFilesystem` interface; add and export `getMirrorFileUris()`.
- `web/src/components/LogsModal.tsx` — import `Capacitor`;
...
- `web/src/lib/__tests__/log-native.test.ts` — add `getUri`
  to the fake fs; add `getMirrorFileUris` tests.
- `web/src/components/LogsModal.stories.tsx` — note native-only
