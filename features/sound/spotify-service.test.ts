...
describe("soundRouter.spotify.nowPlaying via tRPC caller (A14)", () => {
  afterEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("returns mapped now-playing state via the tRPC media router", async () => {
    ...
    const { router } = await import("@app-kit/server");
    const { soundRouter } = await import("./api");
    const testRouter = router({ sound: soundRouter });
    // @ts-expect-error -- no db context needed for unit tests
    const caller = testRouter.createCaller({});

    const result = await caller.sound.spotify.nowPlaying();
...
describe("soundRouter.spotify transport mutations via tRPC caller (A15)", () => {
  afterEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("exposes spotify.play mutation", async () => {
    ...
    await caller.sound.spotify.play();
  ...
  it("exposes spotify.pause mutation", async () => {
    ...
    await caller.sound.spotify.pause();
  ...
  it("exposes spotify.next mutation", async () => {
    ...
    await caller.sound.spotify.next();
  ...
  it("exposes spotify.previous mutation", async () => {
    ...
    await caller.sound.spotify.previous();
  ...
  it("exposes spotify.seek mutation with positionMs input", async () => {
    ...
    await caller.sound.spotify.seek({ positionMs: 45_000 });
