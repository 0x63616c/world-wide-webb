...
describe("soundRouter.spotify.browse via tRPC caller (A16)", () => {
  afterEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("exposes spotify.browse query returning real content", async () => {
    ...
    const result = await caller.sound.spotify.browse();
  ...
  it("throws SpotifyError via tRPC when unconfigured (A3)", async () => {
    ...
    await expect(caller.sound.spotify.browse()).rejects.toThrow("Spotify credentials unconfigured");
  });
});
