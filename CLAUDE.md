...
The iPad runs a thin **native iOS Capacitor "kiosk" shell** (`products/control-center/web/ios`) that loads `https://dashboard.worldwidewebb.co` full-screen, it does **not** bundle the dashboard. The web app deploys via Pulumi/k3s exactly as above; the shell just renders that URL, so **dashboard changes are OTA, no App Store rebuild needed**. Migrated verbatim from the `evee` repo (www-w1a4).
