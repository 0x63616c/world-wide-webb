...
- **No-home-address guard.** `scripts/check-no-home-address.sh` runs via lefthook on every commit — it BLOCKS any staged file containing the home-location name tokens (`kurve`/`wilshire`, any casing). The real home coords + place name live ONLY in 1Password (`HOME_*` env, item "Home Location"); the repo ships a public LA placeholder. Use `env.HOME_*` (api) or `apps/web/src/config/home.ts` (web), never a literal address. The tokens are sanctioned only on the guard's own surface (the script, `lefthook.yml`, `.gitleaks.toml`, this file). Full git history was rewritten to purge the address (CC-3zi / CC-d3j).
