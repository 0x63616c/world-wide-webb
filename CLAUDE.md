...
All secrets live in `secrets/vault.yaml`, SOPS-encrypted with a post-quantum age key (ML-KEM-768 + X25519). This is the single source of truth, migrated off 1Password (www-k8t7). 1Password is now cold-backup only.
...
SOPS_AGE_KEY=$(security find-generic-password -a "$USER" -s "age-world-wide-webb-private-key" -w) \
  sops secrets/vault.yaml   # opens decrypted in $EDITOR, re-encrypts on save+quit
```
...
- **Secrets (from SOPS vault via `prod` environment `AGE_PRIVATE_KEY`):** `ASC_KEY_ID` / `ASC_ISSUER_ID` / `ASC_KEY_CONTENT` (vault: `APP_STORE_CONNECT_API__*`) + `MATCH_PASSWORD` (vault: `APP_STORE_CONNECT_API__MATCH_PASSWORD`); `MATCH_GIT_URL` (`https://github.com/0x63616c/certificates.git` — hardcoded constant) + `MATCH_GIT_BASIC_AUTHORIZATION` (vault: `MATCH_GIT_BASIC_AUTHORIZATION`).
