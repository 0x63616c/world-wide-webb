...
**Epic:** www-kp4k. **Status:** scoped, grounded in real numbers. Foundation for a later media-archive tile + Sonos playback (www-c2pc).
...
- **`products/control-center/media-worker`** (new app + image). Reuses `createWorkerRuntime()` (await-before-reschedule, per-worker error isolation, stats). Imports media domain from `@control-center/api` via a new `./media` barrel, mirroring the existing `worker-deps.ts` seam (interim until `packages/core`).
- **Workers:**
...
## Data model (drizzle, `products/control-center/api/src/db/schema.ts`)
...
## Human-in-the-loop prerequisites (www-kp4k.6, .7)
...
Media-archive tile (www-kp4k.8), retention/cleanup cronJob (www-kp4k.9), Sonos playback (www-c2pc).
