# GOAL — ship the media-ingest pipeline (CC-kp4k) to prod
...
The epic **CC-kp4k** media-ingest pipeline is built, gated, merged to `main` (NO PR), pushed, deployed by bosun, and verified processing a real YouTube link on homelab — **without fake data and without weakening any gate.**

### Executor

Build the autonomous chain (milestones M1–M5 = `CC-kp4k.1`–`.5`) with the **`ship` workflow**, parallelized per its design:

```
Workflow({ name: "ship", args: { resume: "CC-kp4k", push: false } })
```
...
- Every commit subject is `type(media/CC-kp4k.N): …` (commit-msg guard passes).

### Human prerequisites — gate the prod push (CC-kp4k.6, .7)
...
- [ ] Homelab media volume dir exists at the spike-decided path with sufficient free disk (spike CC-kp4k.6 records the number + the default `video_policy`/resolution/retention decision).
...
- [ ] M1–M5 (`CC-kp4k.1`–`.5`) closed; spike `.6` + provisioning `.7` resolved; `.8`/`.9` parked deferred.
- [ ] Epic `CC-kp4k` acceptance met.
