# GOAL, CC-j934: replace bosun+Swarm with Pulumi+k3s, end to end, validated

This file IS the goal condition. Inputs: `docs/k3s-migration/RECON.md` (grounded facts +
Calum's approved decisions, follow them; do not re-litigate). Epic: `bd show CC-j934`.
Execute via the normal ticket lifecycle (`docs/ticket-standards.md`); use the `ship`
workflow on CC-j934 for the build phases once children are filed, parallelizing where
...
- [ ] Child bd tickets filed under CC-j934 (each Ready per ticket-standards: type,
      priority, area, checkbox AC), with deps encoding the phase order. Surface `bd list
      --parent CC-j934` (or epic tree) output.
...
- [ ] All services deployed to k3s with images pulled from GHCR (imagePullSecret),
      Secrets mounted at `/run/secrets/<NAME>`, requests/limits matching the CC-ke9a
...
- [ ] Captive portal reachable on the LAN over TLS with the cert-manager cert (surface
      ...
      OTP flow (CC-q002.17) requires Calum physically present, when everything else is
...
- [ ] All work committed `type(area/CC-xxx)` referencing real child tickets, merged
      worktree→main with NO PR, pushed; `git status` clean, up to date with origin.
- [ ] `bd show CC-j934` closed; every child closed or explicitly deferred with reason.
...
1. **Adopt-only UniFi:** no existing UniFi setting modified or deleted, only additive
   (www-guest VLAN/SSID, guest-portal flip per CC-q002.15). Zero-diff import proof
...
8. Pause with `needs input:` ONLY for: failed Phase-0 spike, the on-device CC-q002.17
