# k3s migration, recon facts + approved decisions (CC-j934)
...
10. **www-guest gets a real isolated VLAN** (own `unifi_network` + VLAN id + subnet, guest
    ...
    the fiddly bit; spike before the on-device cutover (CC-q002.17, needs Calum present).
...
- **NFS:** native `nfs` PV mounts from inside the cluster bypass the OrbStack host
  bind-mount path that HANGS today (likely un-parks media-worker / CC-6mz7). SPIKE: pod
...
- `guest_access` today: portal_enabled=true but `redirect_enabled=false`, external-portal
  mode NOT yet flipped. Future flip (CC-q002.15): auth=custom/external, auth_url =
