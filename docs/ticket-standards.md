...
- [ ] Committed feat(web/tiles/CC-xxx); PR to `main` merged; bd closed
```
...
| **`/finish-ticket`** | work is done, closing out | Run gates capturing output (refuse if red) → verify each AC item (screenshot for UI) → commit `type(area/CC-xxx)` → push branch → open PR to `main` → merge after green checks → `bd close` → harden-as-you-go audit. |
