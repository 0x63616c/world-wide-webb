...
## M5 migration status (www-jtp0.5, 2026-06-14)

The captive portal is becoming a standalone product. This section tracks hostname
and runtime coupling changes. **Until cutover is complete and validated, the CC
Postgres database is the source of truth and the legacy hostname stays live.**
...
**Rollback state:** Control Center DB tables (`portal_guest`, `portal_code`,
`portal_attempt`, `portal_authorization`) are the source of truth. Do NOT drop
them or remove the CC API coupling until at least one successful backup/restore
cycle after cutover is validated (www-jtp0.5.10).
...
**gitops gap (www-j934.17, RESOLVED in www-jtp0.5.9):** `guest_access` was adopted
...
in-container `bun install --frozen-lockfile` (api, web, worker,
