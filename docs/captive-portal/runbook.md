...
## Apple captive sheet (CNA): MUST be raw-IP HTTP (CC-q002.26, 2026-06-12)
...
- The CNA **will NOT render** the SAME page from the real domain
  ...
  Recovering the hostname is tracked (low pri) in **CC-q002.28**.
...
## STATUS UPDATE (2026-06-12, CC-q002.15 + CC-j934.3.2)
...
**created and live via Pulumi** (`infra/unifi`, ticket CC-j934.3.2), not staged
curls. Net state:
...
### Pulumi management of `guest_access` (gitops, ref CC-j934.17)
...
unifi stack is wired into CI gitops (CC-j934.17), the external-portal config must be
