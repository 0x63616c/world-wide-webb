...
> **HISTORICAL (superseded by www-j934).** This checklist was the definition of done for the
> ORIGINAL `bosun` + Docker Swarm deploy build. That stack has been replaced by Pulumi + k3s;
> `bosun`, `deploy.config.ts`, and the `bosun-agent` no longer exist. The checklist is kept only
> as a record of what the Swarm-era build verified. For the CURRENT deploy design and its
> acceptance, see [`docs/deployment-design.md`](./deployment-design.md) and
> [`docs/k3s-migration/GOAL.md`](./k3s-migration/GOAL.md). Do NOT execute the items below against
> the live system, the commands (`bun run bosun …`, `docker stack deploy`) reference deleted code.

> **This checklist WAS the definition of done.** Built the system per the (now historical) Swarm
> design, then drove every item below to `[x]`.
>
