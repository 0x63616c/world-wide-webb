...
Status: **implemented** (CC-rw07). Foundation (`@repo/logger`) + the backend services shipped in CC-rw07.1. Library: **pino**. (bosun was a 4th consumer at the time; it has since been replaced by Pulumi + k8s and removed, so the current backend consumers are api, worker, media-worker.)
