# Media tiles — verified integration notes (CC-c2pc)

These are the **live-tested** facts from the Sonos/Apple TV exploration session (bd epic
`CC-c2pc`, discovery comments). Everything below was exercised against real hardware on
...
1. **Home Assistant** (preferred backbone, already wired in the repo:
   `apps/api/src/integrations/homeassistant/`). Env: `HA_URL`
...
## Done vs missing (as of CC-c2pc discovery)
...
- **Not yet built**: zero code in repo — no `media_player` references in `apps/api/src` or
  `apps/web/src`. No tRPC media router, no Sonos SOAP helper, no tiles, no stories.
