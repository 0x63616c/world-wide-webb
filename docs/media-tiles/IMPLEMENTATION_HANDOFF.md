...
**Target repo:** `0x63616c/control-center` → `products/control-center/web` (React + Vite, TanStack Router/Query, tRPC).
**What this is:** the spec for a set of **media/entertainment tiles** (and their detail modals) to add to the existing wall‑panel dashboard. Build them as real React components in `products/control-center/web/src/components`, reusing the existing primitives and tokens , **do not** copy the HTML prototypes verbatim.
...
2. **Reuse the existing design system** , it already has everything you need:
   - Tokens: `products/control-center/web/src/styles/tokens.css` (surfaces, hairlines, ink ramp, the single blue accent, radii, the `.tile / .pill / .dot / .sw / .range / .chip / .tap / .cap` classes, shimmer/pulse keyframes).
   - Primitives: `products/control-center/web/src/components/ui/` , `Tile`, `TileHeader`, `Pill`, `Stat`, `StatusDot`, `Skeleton`, `Chip`, `ControlTap`, `Modal`, `BorderProgressRing`. Icon set: `products/control-center/web/src/components/Icon.tsx`.
...
```
products/control-center/web/src/components/media/
