# GOAL — Ship the Media/Entertainment Control Center (bd epic CC-c2pc)
...
build with the **Workflow tool** (the `ship` workflow on epic `CC-c2pc`, `push:true`),
...
## 5. Shipped to PROD
Work committed as `type(media/CC-xxx)` per the commit guard, **merged to `main` with NO PR**,
