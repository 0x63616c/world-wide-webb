# CC-cuuw — Access gate for `*.worldwidewebb.co` (Cloudflare Access, bosun-managed)
...
**KioskWatchdog interaction (CRITICAL — exact failure mechanism, verified against the Swift).** The self-recovery watchdog was built assuming an OPEN origin (CC-bwoy). The precise brick path once `dashboard` is gated and the WKWebView's `CF_Authorization` cookie has expired (read against `KioskHealth.swift` + `KioskWatchdog.swift`):
...
- **Gates:** `bun run typecheck`, `bunx biome check .`, `bun run test` (vitest — never bare `bun test`), `bunx knip` (zero dead code — any new exported builder with no consumer yet needs a `/** @public */` tag or a real call site). Commit format `type(area/CC-cuuw)`, e.g. `feat(bosun/CC-cuuw): add reconcileAccess`.
...
Each is a self-contained commit `type(area/CC-cuuw): ...`. Units 1–6 are the safe-to-ship-now machinery; 7–9 are the gated rollout.
