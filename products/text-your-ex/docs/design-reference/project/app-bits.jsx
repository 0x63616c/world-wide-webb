// app-bits.jsx - interactive sub-components: Toggle, Stepper, EvidenceShot, Viewer, MoneyBurst
...
// Amount stepper - big gold number with − / +
function Stepper({ cents, onChange, step = 100 }) {
  const round = (c) => Math.max(step, Math.round(c / step) * step);
  const Round = ({ dir }) => (
    <button
...
// Fake iMessage-style screenshot used as report "evidence". Tappable → full-screen.
function EvidenceShot({ shot, w = 132, onOpen, full = false }) {
...
// Full-screen evidence viewer overlay
function EvidenceViewer({ shots, index, onClose, onIndex }) {
...
// Money burst - flying $ symbols when a slip is logged
function MoneyBurst({ show }) {
  if (!show) return null;
  const bills = Array.from({ length: 14 });
  return (
    <div
      style={{
        position: "absolute",
...
Object.assign(window, { Toggle, Stepper, EvidenceShot, EvidenceViewer, MoneyBurst });
