...
//
// The fill is two layers, both generated at module load (deterministic, ~ms):
//   1. INNER_FILL , varied bento tiling the inner region (the world minus the wall
//      ring) AROUND wherever the real tiles sit. Produced by the bento-fill
//      generator (fillAround), which carves the fill around the real tiles' world
//      rects as reserved holes. Because the real tiles are free-placed (any number,
//      anywhere, any size), this regenerates to fit them with no hand-authored
...
//      (no new tile type), and the varied sizes absorb the edge length with no
//      special corner tile.
...
// Real tiles as reserved holes in INNER-LOCAL coords (world coords shifted in past
// the wall ring). The bento tiles every inner cell EXCEPT these.
const realTileHoles: Rect[] = TILE_REGISTRY.map((t) => ({
  col: t.worldCol - WALL_THICKNESS,
  row: t.worldRow - WALL_THICKNESS,
  cols: t.cols,
  rows: t.rows,
}));

// The inner bento fill in world coords: generate around the real tiles, then shift
// each tile out past the wall ring. ids are stable by index for React keys.
const INNER_FILL: PlaceholderTile[] = fillAround(INNER_COLS, INNER_ROWS, realTileHoles, {
  seed: BENTO_SEED,
...
// Full decorative fill: inner bento (around the real tiles) + the wall ring.
