...
interface CheckboxRowProps {
   ...
   *  the right), mirroring the field label-row error (www-t6yr). */
  errorMessage?: string;
  ...
  return (
    <div className="wwb-check-row">
      ...
      {/* Error shares the checkbox row, right-aligned with the alert icon on its
          right (www-t6yr). Always rendered (role=alert only when populated) so
          toggling it never reflows the submit button below (www-2nrj). */}
      <div
