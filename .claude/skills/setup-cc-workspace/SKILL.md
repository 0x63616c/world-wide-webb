...
description: Use when Calum wants to run/start/spin up control-center locally, set up a dev workspace, view the app, or view the Tilt logs — runs the Tilt dev stack (postgres + api + web + storybook) and opens the app and log UI in cmux views.
---
...
dev stack (postgres, api, web, storybook), then the live app and the Tilt log UI
as cmux browser views. Everything hot-reloads, so edits show up live.
...
| API | http://localhost:4201 | tRPC backend |
