...
1. **Read the ticket.** `bd show <CC-xxx>`. Understand the AC and the type.
...
3. **Claim it.** `bd update <CC-xxx> --claim` (atomically sets you as owner + status `in_progress`).
...
5. **Enter a ticket-id-led worktree.** `EnterWorktree({ name: "<CC-xxx>-<short-slug>" })`. The name MUST lead with the ticket id (worktree guard + convention). All work happens here, never directly on `main`.
