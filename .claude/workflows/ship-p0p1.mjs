...
- TDD: write or extend the vitest test FIRST (red), then implement to green. Web tests sit in products/control-center/web/src/**/__tests__/*.test.tsx; api tests in products/control-center/api/src/__tests__/*.test.ts.
...
- CLEAN UP AFTER YOURSELF: kill any dev server / storybook you start (\`pkill -f "products/control-center/web dev"\`, \`pkill -f storybook\`) before returning; remove any scratch git worktree you created (\`git worktree remove --force <path>\`); never leave background node/vitest processes or temp worktrees behind.
- ZERO fake/hardcoded/placeholder data anywhere (web + api). On unavailable data a tile renders a shimmer Skeleton and keeps retrying , never an invented number. A repo-wide grep for FALLBACK / PLACEHOLDER (uppercase identifiers) must stay empty. DEMO_ is allowed ONLY in products/control-center/api/src/services/network-service.ts and weather-service.ts.
...
- Shared UI primitives live under products/control-center/web/src/components/ui/ (TileHeader, StatCell, Pill, Skeleton, TileWrapper; barrel index.ts) , PREFER them over re-inlining. Tokens in products/control-center/web/src/styles/tokens.css; shimmer keyframes in products/control-center/web/src/styles/globals.css.
