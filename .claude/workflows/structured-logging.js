export const meta = {
  ...
  phases: [
    ...
    { title: 'Commit', detail: 'serial per-ticket commits (type(area/CC-xxx))' },
  ],
}

const EPIC = (args && args.epic) || 'CC-rw07'

// Backend services that produce pod logs (web is browser-side, handled as a thin separate logger).
const SERVICES = [
  { key: 'api', dir: 'apps/api', pkg: '@repo/api', svc: 'control-center_api',
  ...
  { key: 'worker', dir: 'apps/worker', pkg: '@repo/worker', svc: 'control-center_worker',
  ...
  { key: 'media-worker', dir: 'apps/media-worker', pkg: '@repo/media-worker', svc: 'control-center_media-worker',
...
- Commit subjects MUST be type(area/CC-xxx): a Conventional Commit whose scope ends with a real bd ticket id, e.g. feat(logging/CC-rw07.2): ...
`
...
const plan = await agent(
  ...
  `1. Library: pino (already chosen in CC-355t.44). Where the SHARED logger primitive lives so api, worker, media-worker AND bosun can all import it without circular deps or duplicate config. Options: a new packages/logger workspace, or a shared module. Pick one and justify (knip-clean, no cross-app coupling).\n` +
...
const tickets = await agent(
  ...
  `Also: CC-355t.44 ("Adopt pino for structured API logging") is now SUPERSEDED by this epic's api child — close it with "bd close CC-355t.44 --reason superseded" (or add a comment noting the superseding api ticket id if close needs different flags; check bd close --help).\n\n` +
...
const commitSummary = await agent(
  ...
  `Commit-msg guard requires subjects of the form type(area/CC-xxx): desc with a REAL bd ticket id in the scope. Use these tickets:\n` +
  `- epic: ${EPIC}\n` +
