...
const ui = args?.ui !== false // default true; this repo is a UI board
const MAX_FIX_ROUNDS = args?.maxFixRounds ?? 3
const doPush = args?.push === true // default false: conservative, Finalize reports instead
...
- This workflow IS the manual lifecycle parallelized; it follows the taxonomy (feature/bug/refactor/chore/spike/epic → real bd types), the Definition of Ready, and the Definition of Done (gates green + no fake data + screenshot@1366x1024 for UI + committed referencing the ticket + closed). The per-feature acceptance you are handed already encodes it.

REPO MAP:
- products/control-center/web , React board (fixed 1366x1024 wall panel). Shared primitives live under products/control-center/web/src/components/ui/ , PREFER them over re-inlining headers/stats/pills/skeletons. Tokens in products/control-center/web/src/styles/tokens.css; shimmer keyframes in products/control-center/web/src/styles/globals.css.
- products/control-center/api , tRPC backend; services THROW on error/unconfigured rather than returning constants.
- QueryClient (products/control-center/web/src/lib/trpc.ts) retries infinitely (~5s) with refetchOnWindowFocus:false, so tiles recover from outages without fake data.
`
