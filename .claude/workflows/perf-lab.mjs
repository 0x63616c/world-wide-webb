export const meta = {
  name: 'perf-lab',
  description:
    'Builds and benchmarks a throwaway pan-lab prototype that settles the rendering strategy for turning the dashboard into a pannable 2D canvas. ONE Vite+React app, a route per rendering arm (transform-all, transform+content-visibility, transform+windowing, native-scroll, canvas/WebGL), HYBRID tiles (synthetic cheap/medium + real MapLibre + real video), a full-matrix autopan sweep exporting results.json + the knee per arm, and a launcher+results UI Calum drives on the iPad. Beads (epic www-5p4) is the shared mission state.',
  whenToUse:
    'After the perf-lab plan + beads epic are approved (control-center plan idempotent-snacking-clock.md, epic www-5p4). Launch to scaffold the app, build all 5 arms in parallel, run the full stress sweep, finish the launcher, and finalize. Resume by re-launching (closed bd issues are skipped).',
  phases: [
...
// specified in beads epic www-5p4, so there is nothing to re-derive.
const WORK_M = 'sonnet'
const BOOK_M = 'haiku'
