...
REFERENCE PATTERN (evee already does this split): study ${EVEE}/products/control-center/web/src/components/tiles/lights/lights-tile.tsx (container) and lights-tile-view.tsx (pure view), and fan/fan-tile{,-view}.tsx. Replicate that shape.
...
const TILES = [
  { name: 'ClockGreeting', ticket: 'www-ieo', extra: '' },
  { name: 'WeatherNow', ticket: 'www-3cg', extra: '' },
  { name: 'Next12Hours', ticket: 'www-fi3', extra: 'Keep the chart rendering correct (the CHART_INITIAL_WIDTH fix); the ResizeObserver may stay in the view as presentation state.' },
  { name: 'NetworkTile', ticket: 'www-9hi', extra: 'Preserve the design-matched layout (no "Online" word, StatusDot only).' },
  { name: 'TeslaTile', ticket: 'www-4xy', extra: 'Keep the simplified "Tesla" header + lock pill.' },
  { name: 'ControlsTile', ticket: 'www-y12', extra: 'CRITICAL: keep the cooldown poll-pause + optimistic toggle logic in the CONTAINER. ControlsGridView/ETap become pure (data + onToggle props). Do not regress the no-revert behavior or the Lamps On/Off display.' },
  { name: 'DogCamTile', ticket: 'www-vbx', extra: '' },
  { name: 'EventsTile', ticket: 'www-9aa', extra: 'ALSO implement www-8hq in this pass and close it: the nearest (first/soonest) upcoming event renders its day-count in accent green. Tests for the view should assert this.' },
  { name: 'ClimateTile', ticket: 'www-70w', extra: 'ALSO implement www-6k8 and www-2kd in this pass and close them. www-6k8: setpoint->mode threshold heat when target >= 76 (cool <= 70, auto between). www-2kd: optimistic setpoint/mode with a cooldown poll-pause (~5s) so the poll does not snap-back the value mid-interaction , reuse the SAME cooldown approach ControlsTile uses; frontend-only optimistic is sufficient. The container owns the cooldown/optimistic state; the view is pure (target, ambient, mode, action, onSetTarget, onSetMode props).' },
]
...
for (const t of TILES) {
  const folded =
    t.name === 'EventsTile' ? ' Tickets: www-9aa + www-8hq.' :
    t.name === 'ClimateTile' ? ' Tickets: www-70w + www-6k8 + www-2kd.' :
    ` Ticket: ${t.ticket}.`
  const r = await agent(
    `${RULES}\n\nTILE: ${t.name}. Split products/control-center/web/src/components/tiles/${t.name}.tsx into ${t.name}View (pure) + ${t.name} (container) per the pattern above.${folded}\n${t.extra}\nClaim the ticket(s), do the work TDD, run gates, commit, close the ticket(s). Return structured result (tickets = all closed).`,
