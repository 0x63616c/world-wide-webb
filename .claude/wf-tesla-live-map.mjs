export const meta = {
  name: 'tesla-live-map',
  description: 'CC-d3t: replace TeslaTile static SVG with a themed MapLibre + Protomaps live GPS map; build -> adversarial review -> fix',
  phases: [
...
const CTX = `
PROJECT: control-center, a fixed 1366x1024 smart-home wall-panel dashboard. Monorepo: apps/web (React 19 + Vite), apps/api (tRPC).
...
GOAL (bd issue CC-d3t): Replace the hand-drawn static SVG street grid in the Tesla tile with a real, themed, live GPS map.

KEY FACTS:
- Deps already installed in apps/web: maplibre-gl@5.24.0, pmtiles@4.4.1, @protomaps/basemaps@5.7.2.
- A SoCal basemap is on disk at apps/web/public/maps/socal.pmtiles (Protomaps planetiler "protomaps" schema, MVT, maxzoom 15). Vite serves it at the URL path /maps/socal.pmtiles (HTTP range requests). It is gitignored — never commit it.
- The tile's data source apps/api already returns real lat/lon. Live API confirmed returning lat 34.0537, lon -118.2428 (home: Home, LA). Field "place" is a string like "Home".

CURRENT CODE:
- apps/web/src/components/tiles/TeslaTileView.tsx — pure presentational. Contains an inline function TeslaMap() that renders the static SVG grid (the thing to replace). Populated props: { status:"populated", locked, charging, rate, pct, range, odo, climate }.
- apps/web/src/components/tiles/TeslaTile.tsx — container; calls trpc.tesla.get.useQuery and maps data -> TeslaTileView props. Currently drops data.lat/lon/place.
- Tests: apps/web/src/components/tiles/__tests__/TeslaTile.test.tsx and TeslaTileView.test.tsx (vitest + @testing-library/react + jsdom).

THEME TOKENS (apps/web/src/styles/tokens.css) — match these EXACTLY:
...
- Use shared UI primitives in apps/web/src/components/ui (TileHeader, Stat, Pill, Skeleton, Tile) — do not re-inline them. The map overlays (place label, status pill) should reuse existing pill/cap styling already used in the old TeslaMap.
...
1) NEW FILE apps/web/src/components/tiles/TeslaMap.tsx — a self-contained MapLibre map component.
...
2) EDIT apps/web/src/components/tiles/TeslaTileView.tsx
...
3) EDIT apps/web/src/components/tiles/TeslaTile.tsx
...
4) TESTS — keep the suite green (\`bun run test\`):
   ...
   - The old TeslaTile.test.tsx asserts screen.getByText("a local street") and /Parked · Home/. Those labels are GONE. Replace them with assertions on the real \`place\` from MOCK_DATA ("Home") — e.g. expect the place to appear in the map overlay. Keep ALL other assertions (header, pills, charge %, stats) passing.
